...
このドキュメントは、**AI（コーディングアシスタント）に本アプリの仕様を素早く正確に理解させる**ことを目的とした説明書です。実装ファイル（`diagram_v8.12.html`）を全文読まなくても、アーキテクチャ・データモデル・主要関数・拡張時の注意点を把握できるように構成しています。

- 対象実装: `diagram_v8.12.html`（最新版。運用は「最新版を直接更新」）
- **注意**: 本文中の行番号は v8.06 時点のもの。v8.07 以降の追加ぶんで数百行ずれている。関数名で `grep` して現在位置を確かめること。追加ぶんの索引は末尾「付録B: v8.10 で追加された主な関数」を参照。
- **v8.11 以降の差分**: ガントとレーンの見出し固定は `position:sticky` の重ね箱（`#gstick-*` / `g-gantt-*`）に載っている。未保存判定は `docState` / `docFingerprint` / `markSaved` / `isDirty`、確認は `confirmSave4`、保存結果は `toast`。マインドマップのリザーブ領域は `S.mindReserve` と `renderMindReserve` ほか。v8.12 で「図の名称」`#diagram-name` は左サイドバーから `#app-title-bar` 内へ移設（JS 側は `getElementById` 参照のみで変更なし）。
