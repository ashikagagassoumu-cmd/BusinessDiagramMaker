...
このドキュメントは、**AI（コーディングアシスタント）に本アプリの仕様を素早く正確に理解させる**ことを目的とした説明書です。実装ファイル（`diagram_v8.13.html`）を全文読まなくても、アーキテクチャ・データモデル・主要関数・拡張時の注意点を把握できるように構成しています。

- 対象実装: `diagram_v8.13.html`（最新版。運用は「最新版を直接更新」）
- **注意**: 本文中の行番号は v8.06 時点のもの。v8.07 以降の追加ぶんで数百行ずれている。関数名で `grep` して現在位置を確かめること。追加ぶんの索引は末尾「付録B: v8.10 で追加された主な関数」を参照。
- **v8.11 以降の差分**: ガントとレーンの見出し固定は `position:sticky` の重ね箱（`#gstick-*` / `g-gantt-*`）に載っている。未保存判定は `docState` / `docFingerprint` / `markSaved` / `isDirty`、確認は `confirmSave4`、保存結果は `toast`。マインドマップのリザーブ領域は `S.mindReserve` と `renderMindReserve` ほか。v8.12 で「図の名称」`#diagram-name` は左サイドバーから `#app-title-bar` 内へ移設（JS 側は `getElementById` 参照のみで変更なし）。 v8.13 でレーン図の書き出し（PNG/SVG/印刷）は主軸だけを内容まで切り詰める＝`laneExportMainExtent()` を`buildExportSvg` と `expPDF` が使う。交差軸（レーン幅の合計）と見出し帯は残し、トリム位置に外枠の閉じ線を1本描き足す。
...
## 3. ファイル内の区画マップ（`diagram_v8.06.html`・全10166行）
...
### ガントモデル（`S.gantt`）
`defaultGantt()` が生成。時間軸（`startDate`/`endDate`/`scale='hour'|'day'|'week'|'month'`（既定 day・v8.06 で hour 追加）/`colWidth`）、左情報欄の列幅（`nameW`/`descW`/`assigneeW`）、見出し欄 `header`（工事名・自由な「ラベル：値」行・作成日・承認欄）、凡例 `legend` を持つ。工程表用途に対応する汎用カラムモデル。
...
4. 特定機能は §15/§9 の関数名で `grep -nE 'function 名前' diagram_v8.06.html` して該当箇所へ。
...
| 関数/定数 | 役割 |
|---|---|
| `AUTO_W_MAX` / `AUTO_PAD` / `AUTO_LINE_H` / `AUTO_FONT` | 自動幅の上限(260)・文字の左右余白(片側5)・行高(15)・本文サイズ(12) |
...
| 関数 | 役割 |
|---|---|
| `initHandTool()` | `#canvas-wrap` のキャプチャフェーズで mousedown/click/dblclick を握りつぶす。`init()` から呼ぶ |
...
| 関数/要素 | 役割 |
|---|---|
| `<g id="g-lane-hdr">` | 最前面の見出しレイヤ（`g-dividers` の直後） |
| `renderLanes()` | 本体は `g-lanes`、見出しは `g-lane-hdr` へ描き分ける |
| `stickyLaneHeader()` | スクロール量÷`zoomLevel` だけ平行移動。`applyZoom()` の末尾からも呼ぶ |
| `initStickyHeader()` | `#canvas-wrap` の scroll リスナ登録。`init()` から呼ぶ |
| `buildExportSvg` / `buildSaveHtml` | 書き出し前に `g-lane-hdr` の transform を外す |
