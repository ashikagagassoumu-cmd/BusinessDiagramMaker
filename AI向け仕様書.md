...
このドキュメントは、**AI（コーディングアシスタント）に本アプリの仕様を素早く正確に理解させる**ことを目的とした説明書です。実装ファイル（`diagram_v8.10.html`）を全文読まなくても、アーキテクチャ・データモデル・主要関数・拡張時の注意点を把握できるように構成しています。

- 対象実装: `diagram_v8.10.html`（最新版。運用は「最新版を直接更新」）
