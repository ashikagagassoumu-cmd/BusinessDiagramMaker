...
const coverSection = {
  ...
  children: [
    ...
    new Paragraph({
      ...
      children: [new TextRun({ text: '総合編（全5図種対応）', font: 'Meiryo UI', size: 28, color: '404040' })]
    }),
...
const body = [
  ...
  bullet('5種類の図に対応し、作成途中でも図の種類を相互に切り替え可能'),
  ...
  ...imgPara('01_new_diagram_dialog.png', 460, '図1: 新規作成ダイアログ（全5図種）'),
  ...
  bullet('[↓ ﾃｷｽﾄ] - Markdown / OPML / FreeMind として書き出し（マインドマップのみ。10.5）'),
  ...
  para('※ ツールバーのボタンは図の種類に応じて自動的に切り替わります。ガントチャートでは開始日・完了日の入力、時間スケール（時/日/週/月）、折り返し表示、休日（土/日/祝）のチェックが、マインドマップでは配置モード（放射状/ツリー）・表示レベル（1/2/3/全）・フォーカスが追加表示されます。', { color: '555555' }),
  ...
  bullet('レーン - レーンの一覧と管理ボタン（スイムレーン図。第7章）'),
  bullet('プロジェクト設定／見出し・凡例／タスク一覧 - ガントチャート（10.4）'),
  bullet('レイアウト／選択ノードの属性／強調表示／未決・反対一覧 - マインドマップ（10.5）'),
  para('パネルが画面に収まらないときは、パネル内を縦スクロールしてすべての設定にアクセスできます。'),
  para('※ マインドマップでは、画面の右側にも「リザーブ」パネルが表示されます（10.5.6）。', { color: '555555' }),
  spacer(200),
  ...
  para('※ マインドマップだけは例外で、配置しても選択は動きません（選択中のノードが次に追加する枝の親になるため。10.5参照）。', { color: '555555' }),
  spacer(60),
  ...
  bullet('自動リサイズの対象外 - 付箋、区画を持つシェイプ（樹状図のノード）、グループ枠、ステップ図の列、ガント専用のシェイプ'),
  para('※ ガント・マインドマップ等の専用図形は、それぞれ専用の操作（期間の変更など）でサイズを調整します。', { color: '555555' }),
  ...
  para('「プロセス(説明有)」や樹状図の「ノード（ラベル有）」など複数の領域を持つシェイプは、領域ごとにダブルクリックして個別に編集できます。'),
  ...
  para('接続線をダブルクリックすると、ラベル（テキスト）を入力できます。'),
  ...
  tipBox('ガントチャートでは、接続（依存関係）を作成すると FS / SS / FF / SF の種別を選ぶメニューが表示されます（10.4参照）。'),

  heading(2, '5.4 グループ枠（サブプロセス枠）'),
  para('スイムレーン図・フローチャートでは、複数のシェイプをまとめて囲む「グループ」枠を配置できます。'),
  ...
  para('文字揃えは汎用シェイプだけでなく、マインドマップ・樹状図・グループ枠・ガントのバーにも効きます。'),
  ...
  // ===== 7. レーン管理 =====
  heading(1, '7. レーン管理'),
  para('スイムレーン図では、レーンが業務の担当者や部門を表します。'),
  ...
  para('スイムレーン図では、レーンの見出し帯がスクロールに追従して常に表示されます。縦モードでは画面の上端に、横モードでは画面の左端に残るため、下や右へスクロールしてもどのレーンを見ているのか分かります。'),
  ...
  ...imgPara('13_diagram_type_menu.png', 460, '図15: 図の種類切替メニュー（全5種）'),
  heading(2, '9.1 レーンあり図種とレーンなし図種'),
  para('レーンを使うのはスイムレーン図だけです。その他の図種はレーンを使いません。'),
  ...
  heading(3, '10.4.1 見出し欄（表題部・承認欄・凡例）'),
  ...
  heading(3, '10.4.5 見出し・時間軸・タスク名列の固定'),
  ...
  para('※ 折り返し表示（10.4.8）のときは固定しません。段ごとに時間軸ヘッダーとタスク名列を持ち、横スクロール自体が起きないためです。', { color: '555555' }),

  heading(3, '10.4.6 時（1時間ごと）のスケール'),
  ...
  para('ガント工程表では、この「図の名称」が見出しの工事名を兼ねます（10.4.1参照）。見出し編集ダイアログの「工事名」と連動しており、どちらから変更しても同じものが変わります。'),
  ...
  bullet('OPML → マインドマップ（.opml）'),
  bullet('FreeMind → マインドマップ（.mm）'),
